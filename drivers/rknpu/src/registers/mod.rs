// MMIO completion 状态与 IRQ 时间发布，最后修改日期：2026-08-19。
...
pub struct RknpuCore {
    ...
    /// 与 irq_status 同一次发布对应的 IRQ 入口时间，单位为纳秒。
    pub(crate) irq_timestamp_ns: Arc<AtomicU64>,
}
...
impl RknpuCore {
    ...
    pub unsafe fn new(base_addr: NonNull<u8>, core_slot: u8) -> Self {
        Self {
            ...
            irq_timestamp_ns: Arc::new(AtomicU64::new(0)),
        }
    }
    ...
    /// 同时清除 completion 状态和对应 IRQ 时间，防止跨 Task 错配。
    ///
    /// 最后修改日期：2026-08-19。该操作只用于核心尚未绑定新 Task 的边界。
    pub(crate) fn clear_completion_shadow(&self) {
        self.irq_status.store(0, Ordering::Release);
        self.irq_timestamp_ns.store(0, Ordering::Release);
    }

    /// 读取并清除硬件中断，再把归一化状态发布到共享 `irq_status` 原子变量。
    ///
    /// 调用位置包括 IRQ 入口，以及新 Task 下发前清理旧状态的路径。
    ///
    /// 最后修改日期：2026-08-19。返回非零值表示发生完成或错误。该函数运行
    /// 在硬 IRQ 时不输出日志；只执行
    /// MMIO 读取/清除和原子状态发布，后续任务回收由调度 worker 完成。
    pub fn handle_interrupt(&self) -> u32 {
        self.handle_interrupt_at(0)
    }

    /// 在硬 IRQ 路径中把 IRQ 入口时间与完成状态按固定顺序一起发布。
    ///
    /// 最后修改日期：2026-08-19。先写时间戳，再以 Release 发布非零状态；
    /// Worker 对状态执行 Acquire 后读取时间戳，不会观察到新状态配旧时间。
    pub fn handle_interrupt_at(&self, irq_time_ns: u64) -> u32 {
        ...
        if fuzzed != 0 {
            if irq_time_ns != 0 {
                // 重复 IRQ 保留第一次到达时间，直到 Worker 收割该核心。
                let _ = self.irq_timestamp_ns.compare_exchange(
                    0,
                    irq_time_ns,
                    Ordering::Release,
                    Ordering::Relaxed,
...
mod tests {
    ...
    #[test]
    fn handle_interrupt_at_publishes_matching_irq_time() {
        // 最后修改日期：2026-08-19。仿真一次带时间戳的硬 IRQ，确认状态
        // 可见后读取到的是同一次 completion 的入口时间。
        let (core, _mmio) = build_fake_core();
        core.write_mmio_u32(PC_INTERRUPT_STATUS_OFFSET, 0x200);

        let fuzzed = core.handle_interrupt_at(123_456);

        assert_eq!(fuzzed, 0x300);
        assert_eq!(core.irq_status.load(Ordering::Acquire), 0x300);
        assert_eq!(core.irq_timestamp_ns.load(Ordering::Acquire), 123_456);
    }
