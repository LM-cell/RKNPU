...
impl<P: RknpuPlatform> RknpuService<P> {
    /// 创建只使用 completion IRQ Event/Waker 的 RKNPU 服务。
    ///
    /// 最后修改日期：2026-08-22。等待策略不再由调用方选择，避免同一驱动
    /// 同时维护轮询和 IRQ 阻塞两条 completion 推进路径。
    pub fn new(platform: P) -> Self {
        let scheduler = RknpuScheduler::new(&platform);
        Self {
            inner: Arc::new(RknpuServiceInner { platform, scheduler }),
        }
    }
}
