...
const DRM0_NAME: &CStr = c"rockchip";
/// Driver date for DRM device
const DRM0_DATE: &CStr = c"20140818";
/// Driver description for DRM device
const DRM0_DESC: &CStr = c"RockChip Soc DRM";
...
fn drm_version(data: &mut [u8]) -> VfsResult<()> {
    ...
    unsafe {
        // Copy driver name
        let ret = drm_copy_field(data.name as *mut u8, &mut data.name_len, DRM0_NAME.as_ptr() as _);
        if let Err(e) = ret {
        ...
        // Copy driver date
        let ret = drm_copy_field(data.date as *mut u8, &mut data.date_len, DRM0_DATE.as_ptr() as _);
        if let Err(e) = ret {
        ...
        let ret = drm_copy_field(data.desc as *mut u8, &mut data.desc_len, DRM0_DESC.as_ptr() as _);
        if let Err(e) = ret {
...
pub fn copy_from_user(dst: *mut u8, src: *const u8, size: usize) -> VfsResult<()> {
...
pub fn copy_to_user(dst: *mut u8, src: *const u8, size: usize) -> VfsResult<()> {
    let ret = unsafe { user_copy(dst, src, size) };
