...
static int prepare_task(worker_context_t *worker) {
    ...
    worker->resources.submit_template.core_mask = 0x7;
    worker->resources.submit_template.fence_fd = -1;

    /* One logical lane lets three independent submits compete for any physical NPU core. */
...
static int prepare_worker_resources(worker_context_t *worker) {
    if (allocate_dma_buffer(worker->fd, REGCMD_BYTES, 0, &worker->resources.regcmd) != 0 ||
        allocate_dma_buffer(
            worker->fd,
